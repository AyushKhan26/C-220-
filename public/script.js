...
function connect_to_newuser(userId,stream){
...
$(function () {
    ...
    $("#stop_video").click(function(e){
        ...
        if(enabled){
            ...
            html = `<i class='fa fa-video-splash'></i>`
            $('stop_video').toggleClass('background-color: red')
            $('#stop_video').html(html)
        } else {
            myStream.getVideoTracks()[0].enabled = true;
            html = `<i class='fa fa-video'></i>`
            $('stop_video').toggleClass('background-color: blue')
            $('#stop_video').html(html)
        }
    })
